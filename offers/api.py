...
from .models import Coupon, ProductCoupon, Offer, ProductOffer, UserCouponUsage
from .schemas import (
    CouponCreate, CouponUpdate, CouponOut,
    ProductCouponCreate, ProductCouponOut,
...
# Product Coupon Endpoints
@router.post("/product-coupons", response=ProductCouponOut, tags=["Product Coupons"])
def create_product_coupon(request, payload: ProductCouponCreate):
    product_coupon = ProductCoupon.objects.create(
        coupon_id=payload.coupon_id,
        product_id=payload.product_id
    )
    return product_coupon

@router.get("/product-coupons", response=List[ProductCouponOut], tags=["Product Coupons"])
def list_product_coupons(request, product_id: int = None):
    qs = ProductCoupon.objects.all()
    if product_id:
        qs = qs.filter(product_id=product_id)
    return qs

@router.delete("/product-coupons/{product_coupon_id}", tags=["Product Coupons"])
def delete_product_coupon(request, product_coupon_id: int):
    product_coupon = get_object_or_404(ProductCoupon, id=product_coupon_id)
    ...
    try:
        ...
        # Product-specific coupon validation
            ...
            if not product_id:
                return CouponValidationResponse(
                    is_valid=False,
                    message="Product ID is required for product-specific coupons",
                    discount_amount=None,
                    final_price=None
                )
            
            product_coupon = ProductCoupon.objects.filter(
                coupon=coupon,
            ...
            try:
                ...
                if coupon.discount_type == 'percentage':
                    discount = product_price * (coupon.discount_value / Decimal('100'))
                    if coupon.max_discount_amount:
                        discount = min(discount, coupon.max_discount_amount)
                else:  # FIXED
